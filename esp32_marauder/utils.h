...
static inline uint8_t getDRAMUsagePercent() {
...
static inline String hexDump(const uint8_t *buf, size_t len) {
  String out;
...
static inline String byteArrayToHexString(const std::vector<uint8_t>& byteArray) {
  String result;
...
static inline std::vector<uint8_t> hexStringToByteArray(const String& hexString) {
...
static inline void generateRandomName(char *name, size_t length) {
...
static inline const char* generateRandomName() {
...
static inline void generateRandomMac(uint8_t* mac) {
...
static inline String macToString(const Station& station) {
...
static inline String macToString(uint8_t macAddr[6]) {
...
static inline String macToString(const uint8_t macAddr[6]) {
  char macStr[18]; // 17 characters for "XX:XX:XX:XX:XX:XX" + 1 null terminator
  snprintf(macStr, sizeof(macStr), "%02X:%02X:%02X:%02X:%02X:%02X", 
    macAddr[0], macAddr[1], macAddr[2], 
    macAddr[3], macAddr[4], macAddr[5]);
...
static inline void convertMacStringToUint8(const String& macStr, uint8_t macAddr[6]) {
...
static inline IPAddress getNextIP(IPAddress currentIP, IPAddress subnetMask) {
...
static inline IPAddress getPrevIP(IPAddress currentIP, IPAddress subnetMask, uint16_t stepsBack) {
  // Convert IPAddress to uint32_t
  uint32_t ipInt = (currentIP[0] << 24) | (currentIP[1] << 16) | (currentIP[2] << 8) | currentIP[3];
  uint32_t maskInt = (subnetMask[0] << 24) | (subnetMask[1] << 16) | (subnetMask[2] << 8) | subnetMask[3];
...
static inline uint16_t getNextPort(uint16_t port) {
  return port + 1;
}
