...
// CH32V003: IO Port Expander replaces TFT_BL Pin
    ...
    const uint8_t BL_LEVELS[] = {0, 17, 34, 51, 68, 85, 102, 119, 136, 153, 170, 187, 204, 221, 238, 255};

    uint8_t BL_NUM_LEVELS = sizeof(BL_LEVELS);
    // Dummy Functions, should never be called but are here just in case

    void brightnessInit() { };   // brightnessInit Fallthrough
    ...
    void brightnessSet(uint8_t level) {
      log_i("HAS_CH32V003 brightnessSave set = %d : %d", level, BL_LEVELS[level]);
      ...
      // log_i("HAS_CH32V003 brightnessSave SET = %d : %d", bl_level_idx, BL_LEVELS[bl_level_idx]);
