...
void Buffer::open() {
  if (bufA == NULL && bufB == NULL) {
    bufLength = ((heap_caps_get_largest_free_block(MALLOC_CAP_8BIT | MALLOC_CAP_SPIRAM) * 0.95) / 2);
    ...
    bufB = (uint8_t*)heap_caps_malloc(bufLength, MALLOC_CAP_8BIT | MALLOC_CAP_SPIRAM);
...
void Buffer::save(fs::FS* fs) {
  ...
  if (!file) {
    Serial.printf("Failed to open file: %s", fileName);
    //useSD = false;
    return;
  }
