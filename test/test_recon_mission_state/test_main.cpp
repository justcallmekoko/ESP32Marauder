...
void test_consumes_only_new_entries() {
  ...
  ReconRange first = state.consume(ReconSource::WIFI_AP, 4);
  ReconRange second = state.consume(ReconSource::WIFI_AP, 6);
...
void test_handles_source_lists_being_cleared() {
  ReconMissionState state;
  state.consume(ReconSource::BLE, 5);
  ReconRange after_clear = state.consume(ReconSource::BLE, 2);
...
void test_tracks_sources_independently() {
  ReconMissionState state;
  state.consume(ReconSource::WIFI_AP, 3);
  ReconRange stations = state.consume(ReconSource::WIFI_STATION, 2);
